...
- `id`：适当大小的无符号整数类型（例如 `MEDIUMINT UNSIGNED`）。
...
- `dept_id`：适当大小的无符号整数类型（例如 `SMALLINT UNSIGNED`）。
...
- `pk_grade_ID`：适当大小的无符号整数类型（例如 `INT UNSIGNED`），设置为 `AUTO_INCREMENT` 并作为主键。
...
```sql
CREATE TABLE grades (
    pk_grade_ID INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
- 使用 phpMyAdmin 导入数据
    ...
    ![选择表](/cse330/module3/table.png)

    - 选择完表后点击上方的import 按钮
    ![导入按钮](/cse330/module3/import.png)

    - 上传我提供的对应的csv文件，Format 选择 `CSV`，然后点击 `Import` 按钮
    ![上传文件](/cse330/module3/startimport.png)
